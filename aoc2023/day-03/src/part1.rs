...
fn parse_input(input: &str) -> (HashSet<(i32, i32)>, Vec<PartNumber>) {
    ...
    return (syms_coords, part_numbers);
}

pub fn solution(input: &str) -> u32 {
    let (syms_coords, part_numbers) = parse_input(input);

    let total = part_numbers
        .iter()
        .filter(|part_num| { part_num.surrounding.intersection(&syms_coords).next().is_some() })
